...
def parse_options():
    """
    command line options parser: initiates the option parser and returns the parsed object
    """
    ...
    parser = optparse.OptionParser(usage=return_help())
    ...
    h_evec = 'switch on eigenvector computation'
    ...
    group = optparse.OptionGroup(parser, "Processing directives", "Select exactly one of these options")
    ...
    # hsf options
    group = optparse.OptionGroup(parser, "Required argument if using --hsfid")
    group.add_option('--hemi', dest='hemi', help=h_hemi)
    parser.add_option_group(group)

    group = optparse.OptionGroup(parser, "Optional argument if using --hsfid")
    ...
    # shapedDNA options
    group = optparse.OptionGroup(parser, "ShapeDNA parameters")
...
def check_options(parser, options):
    ...
    try:
        ...
        if e.errno != os.errno.EEXIST:  # directory exists (but that's OK)
            raise e
        pass
...
def get_hsf_surf(options):
    ...
    #
    astring2 = ' '.join(options.hsfid)
    subjdir = os.path.join(options.sdir, options.sid)
    hsf = os.path.join(options.options.sdir, options.sid, 'mri', options.hemi + '.' + options.hsfseg + 'Labels-' + options.hsflabel + '.' + options.hsfver + options.hsfsfx + '.mgz')
...
def compute_shapeDNA_tria(surf, options):
    ...
    # imports
    import os
    from brainprintpython.laplaceTria import laplaceTria
    from brainprintpython.pyUtilsIO import importVTK
...
# write EV file
def write_ev(v, t, ev, evec, options):
    """
    a function to write EV files
    """
    
    # imports
    import os
    from brainprintpython.pyUtilsIO import exportEV
    ...
    evDict['Degree'] = 1
    evDict['Dimension'] = 2
    evDict['Elements'] = len(t)
    evDict['DoF'] = len(v)
    evDict['NumEW'] = options.num
    evDict['Eigenvalues'] = ev
    evDict['Eigenvectors'] = evec

    # determine outev
    outev = options.outsurf + '.ev'

    # write to EV file
    exportEV(evDict, outev)


# write surface file
def write_surf(v, t, options):
    """
    a function to write vtk surfaces
    """    

    # imports
    from brainprintpython.pyUtilsIO import exportVTK

    # write to VTK file
    exportVTK(v, t, options.outsurf)
...
if __name__ == "__main__":
    ...
    # run shapeDNA tria
    v, t, ev, evec = compute_shapeDNA_tria(surf, options)
    
    # write ev and surf
    write_ev(v, t, ev, evec, options)
    write_surf(v, t, options)
