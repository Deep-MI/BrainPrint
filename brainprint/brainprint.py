...
def _parse_options():
    ...
    parser = argparse.ArgumentParser(
        description="This program conducts a brainprint analysis of FreeSurfer output.",
        add_help=False,
    )
    ...
    if args.sid is None:
        print(
            "ERROR: the --sid argument is required, exiting. Use --help to see details."
        )
        sys.exit(1)

    if args.sdir is None:
        print(
            "ERROR: the --sdir argument is required, exiting. Use --help to see details."
        )
        sys.exit(1)
