...
# -*- coding: latin-1 -*-
"""
brainprint_postproc - a script for the post-processing of brainPrint results

type 'brainprint_postproc.get_help()' for info.
"""
...
def get_help(print_help=True):
    ...
    brainprint_postproc.py --file=/path/to/subject-directory/brainPrintOutputFile.csv
    --vol=1 --lin --asy=euc
    ...
    brainprint_postproc.py --list=/path/to/list-of-brainPrintOutputFiles.txt --vol=1
...
def show_history():
    """
    a function to show the revision history of this module
    ...
    # Date:             Nov-19-2019
    # Version:          2.0
    # Comment:          Simplification. No more Python 2.x compatibility.
    #                   Harmonization.
    #
    # Author:           Kersten Diers
    # Date:             Jul-18-2019
    # Version:          1.0
    # Comment:          Modularized design of the script; pyPostProc can
    #                   now be imported as a function
    #
...
def parse_options():
    ...
    group.add_option('--file', dest='file', help=h_file, default=None)
    group.add_option('--list', dest='list', help=h_list, default=None)
    ...
    group.add_option('--asy', dest='asy', help=h_asy, default=None)
    group.add_option('--covfile', dest='covfile', help=h_covfile, default=None)
    group.add_option('--out', dest='out', help=h_out, default=None)
    group.add_option('--outcov', dest='outcov', help=h_outcov, default=None)
    parser.add_option_group(group)
...
def check_options(options):
    """
    a function to check command line inputs
    """
...
def compute_postproc(options):
    ...
    def provide_covariance(covInput, evalSize):
        ...
        covFile = np.genfromtxt(covInput, dtype=['S32'] + ['float'] * (np.divide((evalSize[0] ** 2 - evalSize[0]), 2) + evalSize[0]), delimiter=',')
        ...
        expectedSize = (evalSize[1], np.divide((evalSize[0] ** 2 - evalSize[0]), 2) + evalSize[0])
    ...
    print("-----------------------------------------------------------------")
    print("BrainPrint post-processing")
    print("-----------------------------------------------------------------")
    ...
    check_evalSize = list()
    for k in list(data.keys()): check_evalSize.append(
        np.shape(data[list(data.keys())[0]]['evals']) == np.shape(data[k]['evals']))
    ...
    if options.vol == 1:
        ...
        for f in options.csvfiles:
            ...
            for ni, nj in enumerate(data[f]['names']):
                if re.search("3d", nj):
                    tmp[:, ni] = data[f]['evals'][:, ni] * data[f]['volume'][ni] ** (np.divide(2.0, 3.0))
                else:
                    tmp[:, ni] = data[f]['evals'][:, ni] * data[f]['surface'][ni] ** (np.divide(2.0, 2.0))
    ...
    if options.lin:
        ...
        for f in options.csvfiles:
            ...
            tmp = np.divide(tmp, (np.tile(np.reshape(np.r_[0:tmp.shape[0]], (tmp.shape[0], 1)), (1, tmp.shape[1])) + 1.0))
        ...
        for f in list(data.keys()):
            ...
            for p in list(pairs_idx.keys()):
                ...
                if not np.isnan(data[f]['evalsNormLin'][1:(options.nEV + 1), pairs_idx[p]]).any():
                    if options.asy == "euc":
                        dist.update({p: np.float(
                            di.euclidean(data[f]['evalsNormLin'][1:(options.nEV + 1), pairs_idx[p][0]],
                                         data[f]['evalsNormLin'][1:(options.nEV + 1), pairs_idx[p][1]]))})
                    elif options.asy == "mah":
                        dist.update({p: np.float(
                            di.mahalanobis(data[f]['evalsNormLin'][1:(options.nEV + 1), pairs_idx[p][0]],
                                           data[f]['evalsNormLin'][1:(options.nEV + 1), pairs_idx[p][1]],
                                           np.matrix(covMat[p][1:(options.nEV + 1), 1:(options.nEV + 1)]).I))})
    ...
    for f in options.csvfiles:
        ...
        if options.asy is not None:
            with open(outfilebase + '_distances' + outfileext, WRITECSVSPECIFIER) as outfilehandle:
                csvwriter = csv.DictWriter(outfilehandle, fieldnames=list(data[f]['dist'].keys()), delimiter=',',
...
# ------------------------------------------------------------------------------
# run brainprint_postproc (as a funtcion)
